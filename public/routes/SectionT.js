...
router.post('/announcement', (req, res) => {
    const { sectionID, Announcement } = req.body;

    const query = `UPDATE section SET announcement = ? WHERE sectionid = ?`;
    db.query(query, [Announcement, sectionID], (err, results) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ success: false, message: 'Internal server error.' });
        }
...
router.post('/content2', (req, res) => {
    const { sectionID, ContentTitle, Content } = req.body;

    const query = `INSERT INTO content (sectionid, coursecontenttitle, coursecontent) VALUES (?, ?, ?)`;
    db.query(query, [sectionID, ContentTitle, Content], (err, results) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ success: false, message: 'Internal server error.' });
        }
...
router.post('/enrolledstudents2', (req, res) => {
    const { sectionID } = req.body;

    const query = `SELECT s.studentid, s.name FROM student s, admit a WHERE s.studentid = a.studentid and a.status like 'Requested' and a.sectionid = ?`;
    db.query(query, [sectionID], (err, results) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ success: false, message: 'Internal server error.' });
...
router.post('/accept', (req, res) => {
    ...
    const query = `UPDATE admit SET status = 'Accepted' WHERE studentid = ? and sectionid = ?`;
    db.query(query, [studentID, sectionID], (err, results) => {
        if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ success: false, message: 'Internal server error.' });
        }
